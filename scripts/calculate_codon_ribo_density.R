...
overhang <- 99 # nt around each codon for codon density calculation
...
calculateCodonDensity <- function (codon) {
  codonlocs <- (
    ...
    %>% resize(., width=2*overhang, fix='center')  # resize to get 100nt each side
  )
  ...
  codon.cvg.df %>% summarize_each(funs(mean))  # calculate mean across all codon instances
}

codon.ribo.density <- lapply(codons, calculateCodonDensity)
names(codon.ribo.density) <- codons
codon.ribo.density <- bind_rows(codon.ribo.density, .id='codon')
write_tsv(format(codon.ribo.density, digits=4),
          paste0(
            'processeddata/',
            samplename,
            '/codon.ribo.density.tsv'
          )
          )
